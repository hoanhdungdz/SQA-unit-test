"""
=============================================================
 WHITE-BOX UNIT TEST – Module: Chatbot & Chats (Mạnh)
 File target : chatbots/views.py  |  chats/views.py
 Method      : White-Box Testing (Branch / Path / Condition)
 Total TCs   : 10  (khớp 1-1 với agoda_sheet_chatbot.md)
...
class TestChatbotsViewsWhiteBox:
    ...
    def setup_method(self):
        ...
        # Prefix đúng trong urls.py là /api/chatbots/ (có chữ s)
        self.ask_url           = '/api/chatbots/ask/'
        self.create_chat_url   = '/api/chatbots/new/'
        self.get_messages_url  = '/api/chatbots/messages/'
    ...
    def test_TC_BOT_BE_01_ask_chatbot_missing_question_returns_400(self):
        ...
        response = self.api_client.post(
            ...
            format='json'  # chỉ dùng format, không dùng content_type cùng lúc
        )
    ...
    def test_TC_BOT_BE_05_create_new_chat_user_already_has_chat_id_returns_existing(self):
        ...
        # chat_id là IntegerField trong model → phải dùng số nguyên
        existing_chat_id_value = 99001
        ...
        assert str(existing_chat_id_value) in str(response.content)
...
# ==============================================================================
# EXTRA TEST CASES – Exposing real bugs (expected to FAIL)
# ==============================================================================

@pytest.mark.django_db(transaction=True)
class TestChatsViewsBugExposure:
    """
    EXTRA WHITEBOX TEST – Bug Exposure Cases (Expected to FAIL)
    These test cases reveal real validation gaps in the production code.
    They are documented with Status=FAIL in the test case sheet.
    """
    ...
    def setup_method(self):
        ...
        self.user_a = User.objects.create_user(
            username="bug_user_a", password="PassA123!"
        )
        self.user_b = User.objects.create_user(
            username="bug_user_b", password="PassB123!"
        )

    # ---------------------------------------------------------------
    # TC_BOT_BE_BUG_01 — SendMessageView: empty text not validated
    # ---------------------------------------------------------------
    def test_TC_BOT_BE_BUG_01_send_empty_text_should_be_rejected(self):
        """
        [Test Case ID: TC_BOT_BE_BUG_01]
        WHITE-BOX BUG EXPOSURE: SendMessageView.create()
          Bug: The view does NOT validate whether 'text' is empty before saving.
          Expected behavior: HTTP 400 — empty message should be rejected.
          Actual behavior  : HTTP 200 — empty message saved to DB (BUG).
        Technique  : Statement Coverage – missing validation branch
        Expected   : HTTP 400. isSuccess=False.
        Actual     : HTTP 200. Message with empty text inserted into DB.
        Status     : FAIL (Bug confirmed in production code)
        """
        # Pre-condition: valid conversation exists
        ...
        # Input: empty text string — should be rejected by validation
        payload_empty_text = {
            "conversation_id": conversation.id,
            "text": ""   # ← empty string, no content
        }

        response = self.api_client.post(
            self.send_msg_url,
            data=payload_empty_text,
            format='json'
        )

        # EXPECTED: server rejects empty text with 400
        # ACTUAL  : server accepts it and saves empty message → returns 200
        # → This assertion WILL FAIL, exposing the missing validation bug
        assert response.status_code == 400, (
            f"BUG: SendMessageView allows empty text. "
            f"Got {response.status_code}, expected 400."
        )

    # ---------------------------------------------------------------
    # TC_BOT_BE_BUG_02 — GetOrCreateConversation: self-conversation allowed
    # ---------------------------------------------------------------
    def test_TC_BOT_BE_BUG_02_user_cannot_create_conversation_with_themselves(self):
        """
        [Test Case ID: TC_BOT_BE_BUG_02]
        WHITE-BOX BUG EXPOSURE: GetOrCreateConversationView.post()
          Bug: The view does NOT check whether 'user_id' equals
               the authenticated user's own ID. A user can create
               a conversation with themselves.
          Expected behavior: HTTP 400 — "Cannot start a conversation with yourself."
          Actual behavior  : HTTP 200 — self-conversation created in DB (BUG).
        Technique  : Branch Coverage – missing self-reference guard clause
        Expected   : HTTP 400. isSuccess=False.
        Actual     : HTTP 200. Conversation with user1=user2=self created.
        Status     : FAIL (Bug confirmed in production code)
        """
        self.api_client.force_authenticate(user=self.user_a)

        # Input: user_id = own ID (same as authenticated user)
        payload_self_reference = {
            "user_id": self.user_a.id   # ← same as request.user
        }

        response = self.api_client.post(
            self.get_or_create_url,
            data=payload_self_reference,
            format='json'
        )

        # EXPECTED: server rejects self-conversation with 400
        # ACTUAL  : server creates a conversation where user1 == user2 → returns 200
        # → This assertion WILL FAIL, exposing the missing guard clause
        assert response.status_code == 400, (
            f"BUG: GetOrCreateConversationView allows self-conversation. "
            f"Got {response.status_code}, expected 400."
        )
